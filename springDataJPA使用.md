[TOC]
# SpringDataJPA 使用
**根据SpringMVC+SpringDataJPA+Hibernate搭建教程 搭建环境**
## 构建自己的repo 实现逻辑删除（构建删除表存储删除的数据）
1. 构建repo
```java
@NoRepositoryBean
...
//                .apis(RequestHandlerSelectors.basePackage("com.jun.controller"))
    ...
    private ApiInfo apiInfo() {
        return new ApiInfoBuilder()
                ...
                .termsOfServiceUrl("http://localhost:8888/swagger-ui.html") // 将“url”换成自己的ip:port
...
## 使用Lombok简化代码
1. eclipse
      下载(https://projectlombok.org/)->安装->重启(报错->clean一下项目)
...
5. 使用：在dao的类或者其方法上 使用 注解：@DataSource("mysql2")

### 不同库（一个H2、2个mysql）数据源实践
...
5. *启动服务器访问：http://localhost:8080/demo3/findall -->http://localhost:8080/demo3/copy
